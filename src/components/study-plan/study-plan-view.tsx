...
/**
 * The active plan and the subject list arrive from the server. Only generating
 * a new plan — a mutation the student triggers — still goes over the network.
 *
 * `daysRemaining` is computed on the server too: deriving it here would run
 * once during SSR and again on hydration, against two different clocks.
 */
export function StudyPlanView({
  ...
  const [weeks, setWeeks] = useState<ReturnType<typeof groupByWeek>>(
    initialPlan ? groupByWeek(initialPlan.items) : [],
  );
  ...
  // Form state — open by default when there is no plan to show yet.
  const [showForm, setShowForm] = useState(!initialPlan);
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            isDone
                              ? "border-tone-green-line bg-tone-green-soft/70"
                          ...
                          <div className="min-w-0 flex-1">
                            <p
                              className={cn(
                                ...
                                isDone ? "text-tone-green-ink" : "text-foreground",
                              )}
                            >
