...
export default async function LessonPracticeResultPage({
  params,
  searchParams,
}: {
  params: Promise<{
  ...
  if (!attemptId) redirect(`/subjects/${subjectSlug}/${topicSlug}/lessons/${lessonId}`);
  ...
  if (!attempt) {
    redirect(`/subjects/${subjectSlug}/${topicSlug}/lessons/${lessonId}`);
  }
  ...
  const lessonHref = `/subjects/${subjectSlug}/${topicSlug}/lessons/${lessonId}`;
  const practiceHref = `${lessonHref}/practice`;
